...
WIRED 🧠 CHAOS META (Project NEURO META X | NMX 🧠) is an agentic AI architecture for multi-agent orchestration, risk-aware automation, and structured decision systems.

At a high level, NMX focuses on:
- Coordinated agent swarms instead of single-model assistants
- Governance, risk, and auditability as first-class concerns
- Structured, schema-aligned outputs for both humans and machine agents

For the full NEURO META X narrative, design principles, and capability breakdown, see [`docs/neuro-meta-x.md`](docs/neuro-meta-x.md).
